...
export const metadata: Metadata = {
  title: "Catalogue kiến thức làm đẹp | 360dep.vn",
        ...
        <header className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
          <div className="max-w-3xl">
            <div className="inline-flex min-h-8 items-center gap-2 rounded-full bg-rose-50 px-3 text-xs font-black uppercase tracking-[0.18em] text-rose-700 dark:bg-rose-950/40 dark:text-rose-300">
              <Layers3 className="h-4 w-4" /> 14 catalogue · 4 nhóm
            </div>
            ...
            <p className="mt-4 text-lg leading-relaxed text-slate-600 dark:text-slate-300">Dùng finder nếu bạn chưa rõ nên đọc gì, hoặc duyệt toàn bộ catalogue theo bốn nhóm bên dưới.</p>
          </div>
              ...
              <section key={group.slug} aria-labelledby={`group-${group.slug}`}>
                ...
                <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                  {sections.map((section) => (
                    <Link key={section.slug} href={`/catalogue/${section.slug}`} className="group flex min-h-52 flex-col rounded-3xl border border-slate-100 bg-white p-6 shadow-sm transition hover:-translate-y-0.5 hover:border-rose-200 hover:shadow-md dark:border-slate-800 dark:bg-slate-900 dark:hover:border-rose-900">
