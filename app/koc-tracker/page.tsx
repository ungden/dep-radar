...
export default function KocTrackerPage() {
  ...
  const [credibilityFilter, setCredibilityFilter] = React.useState<CredibilityFilter>("all")
  const [reachFilter, setReachFilter] = React.useState<ReachFilter>("all")
  const [verificationFilter, setVerificationFilter] = React.useState<VerificationFilter>("all")
  const [sortMode, setSortMode] = React.useState<SortMode>("influence")
      ...
      <div className="container mx-auto px-4 md:px-6">
        ...
        >
          <h1 className="text-3xl md:text-4xl font-display font-bold text-slate-900 dark:text-slate-50 mb-4">
            Bảng xếp hạng KOL/KOC beauty
          </h1>
          <p className="text-lg text-slate-600 dark:text-slate-400 max-w-3xl">
            Mặc định xếp theo sức ảnh hưởng: tổng độ phủ đa nền tảng, độ hiện diện beauty và tín hiệu verified. Có thể đổi sang độ tin cậy nếu bạn muốn ưu tiên chuyên môn/review sạch hơn.
          </p>
        </motion.div>
        ...
        >
          ...
          <MetricCard icon={ShieldCheck} label="Đã xác minh" value={kols.length ? kols.filter((kol) => kol.verified).length : "..."} />
        </motion.div>
        ...
        >
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-[minmax(260px,1.2fr)_190px_190px_190px_170px_160px]">
            ...
            >
              ...
              <option value="balanced">Ảnh hưởng + tin cậy</option>
              <option value="credibility">Độ tin cậy</option>
            ...
            >
              <option value="all">Tất cả mức tin cậy</option>
