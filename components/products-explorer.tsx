...
const CompareBar = dynamic(() => import("@/components/compare-bar").then((module) => module.CompareBar))
const CompareButton = dynamic(() => import("@/components/compare-button").then((module) => module.CompareButton))
...
export function ProductsExplorer({
  ...
  initialEvents: CreatorProductEvent[]
  initialOffers: ProductOffer[]
  initialFilters?: Record<string, string>
}) {
  const [searchQuery, setSearchQuery] = React.useState(initialFilters.q ?? "")
  const [selectedNeed, setSelectedNeed] = React.useState<string | null>(initialFilters.need ?? null)
  const [selectedCatalogue, setSelectedCatalogue] = React.useState(initialFilters.catalogue ?? "all")
  const [selectedTaxonomy, setSelectedTaxonomy] = React.useState(initialFilters.category ?? "all")
  const [selectedGroup, setSelectedGroup] = React.useState(initialFilters.group ?? "all")
  const [selectedBudget, setSelectedBudget] = React.useState(initialFilters.budget ?? "all")
  const [sortMode, setSortMode] = React.useState<SortMode>((initialFilters.sort as SortMode) ?? "science")
  const products = React.useMemo(() => initialProducts.map(productWithTaxonomy), [initialProducts])
  const timelineEvents = initialEvents
  const productOffers = initialOffers

  React.useEffect(() => {
    const params = new URLSearchParams()
    if (searchQuery) params.set("q", searchQuery)
    if (selectedNeed) params.set("need", selectedNeed)
    if (selectedCatalogue !== "all") params.set("catalogue", selectedCatalogue)
    if (selectedTaxonomy !== "all") params.set("category", selectedTaxonomy)
    if (selectedGroup !== "all") params.set("group", selectedGroup)
    if (selectedBudget !== "all") params.set("budget", selectedBudget)
    if (sortMode !== "science") params.set("sort", sortMode)
    const query = params.toString()
    window.history.replaceState(null, "", query ? `/products?${query}` : "/products")
  }, [searchQuery, selectedNeed, selectedCatalogue, selectedTaxonomy, selectedGroup, selectedBudget, sortMode])
  ...
  React.useEffect(() => {
    trackEvent("shortlist_viewed", {
      ...
      concern: selectedNeed,
      category: selectedTaxonomy,
    })
  }, [filteredProducts.length, selectedNeed, selectedTaxonomy])
  ...
  return (
    <div className="min-h-screen bg-slate-50 py-10 dark:bg-slate-950 md:py-12">
      <div className="container mx-auto px-4 md:px-6">
        ...
        <details className="group mb-8 rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
          <summary className="cursor-pointer list-none font-display text-lg font-bold text-slate-900 marker:hidden dark:text-slate-50">
            Xem ma trận research chuyên sâu
          </summary>
          <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <h2 className="font-display text-xl font-bold text-slate-900 dark:text-slate-50">
        ...
        <section className="mb-6 space-y-4 rounded-2xl border border-slate-100 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        ...
        {filteredProducts.length > 0 ? (
          ...
          <div className="rounded-3xl border border-slate-100 bg-white py-20 text-center shadow-sm dark:border-slate-800 dark:bg-slate-900">
        ...
        <div className="mt-auto border-t border-slate-100 pt-4 dark:border-slate-800">
          <div className="mb-3 flex items-center justify-between gap-3">
            ...
            {product.reviews > 0 ? (
              ...
              <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">Chưa có đánh giá</span>
            )}
          </div>
