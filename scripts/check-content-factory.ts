...
const sourceReadinessMigration = fs.readFileSync("supabase/migrations/20260829112634_enforce_content_signal_source_readiness.sql", "utf8")
const repairedSignalMigration = fs.readFileSync("supabase/migrations/20260829112811_requeue_repaired_content_signals.sql", "utf8")
const planner = fs.readFileSync("lib/content-factory/planner.ts", "utf8")
...
assert.ok(planner.includes('ilike("source_platform", "%tiktok%")'), "New creator-evidence signals must be TikTok-only")
assert.ok(planner.includes('ilike("source_type", "%tiktok%")'), "Evidence slots must not select paused social sources")
assert.ok(planner.includes("hasProductProvenance(product)"), "Creator-evidence signals must have product provenance before planning")
assert.ok(sourceReadinessMigration.includes("insufficient_product_provenance"), "Existing signals without product provenance must fail closed")
assert.ok(repairedSignalMigration.includes("insufficient_accessible_sources"), "Repaired signal provenance must be requeued without deleting audit history")
...
assert.ok(gemini.includes("isContentProviderConfigured"))
...
assert.ok(pipeline.includes("social_source_paused_tiktok_focus"))
assert.ok(pipeline.includes("GEMINI_API_KEY is not configured"))
