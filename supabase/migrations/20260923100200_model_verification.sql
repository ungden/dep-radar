-- Model services are only open to an identity-verified account.
--
-- Checked in two places, which between them cover every door:
--   * the listing trigger, so an unverified freelancer cannot list or re-activate
--     a model service (insert or update of pro_services);
--   * availability_problem(), which create_booking, accept_offer, send_offer,
--     request_reschedule and free_slots all ask before anything is sold. A
--     verification that is later revoked therefore stops new bookings at once,
--     without anyone having to remember to switch the listing off.
...
begin
  ...
  select studio_address, categories, identity_status into pro from public.pros where id = new.pro_id;
  ...
  if tpl.requires_verification and new.active and pro.identity_status <> 'verified' then
    raise exception 'Dịch vụ người mẫu chỉ mở cho tài khoản đã xác minh danh tính.'
      using errcode = 'check_violation';
  end if;
  return new;
end $$;

-- Same body as 20260918040200, plus the verification line. Still security definer
-- (20260919111530): it has to see other customers' bookings to answer truthfully.
...
begin
  ...
  if tpl.requires_verification and pro.identity_status <> 'verified' then
    return 'Dịch vụ người mẫu chỉ mở cho tài khoản đã xác minh danh tính.';
  end if;
...
-- `create or replace` keeps the grants; restate them so a fresh database and
-- the privilege test in rules.sql read the same thing.
revoke all on function public.check_listing_allowed() from public, anon, authenticated;
